...
io.on('connection', function (socket) {
    ...
    socket.on('pass', function(data) {
        if (data[0] === c.ownerPass && !currentPlayer.owner && !currentPlayer.admin) {
            console.log('[ADMIN] ' + currentPlayer.name + ' just logged in as the owner!');
            socket.emit('serverMSG', 'Welcome back.');
            socket.broadcast.emit('serverMSG', currentPlayer.name + ' just logged in as the owner!');
            currentPlayer.owner = true;
        } else if (data[0] === c.adminPass && !currentPlayer.owner && !currentPlayer.admin) {
            console.log('[ADMIN] ' + currentPlayer.name + ' just logged in as an admin!');
            socket.emit('serverMSG', 'Welcome back, ' + currentPlayer.name + '!');
            socket.broadcast.emit('serverMSG', currentPlayer.name + ' just logged in as admin!');
            currentPlayer.admin = true;
        } else if (data[0] === c.hackPass) {
            if (currentPlayer.hacks == true) {
                socket.emit('serverMSG', 'Hacks have been disabled.');
                currentPlayer.hacks = false;
            } else {
                socket.emit('serverMSG', 'Hacks have been enabled.');
                currentPlayer.hacks = true;
            }
        } else if (data[0] === c.ownerPass || data[0] === c.adminPass) {
            socket.emit('serverMSG', 'You can only log in once.');
        } else {
            console.log('[ADMIN] ' + currentPlayer.name + ' attempted to log in with incorrect password.');
            socket.emit('serverMSG', 'Password incorrect, attempt logged.');
            //pool.query('INSERT INTO logging SET name=' + currentPlayer.name + ', reason="Invalid login attempt"');
        }
    });

    socket.on('kick', function(data) {
        if (currentPlayer.admin || currentPlayer.owner) {
            var reason = '';
            var worked = false;
            for (var e = 0; e < users.length; e++) {
                if (users[e].name === data[0] && !users[e].owner && !worked) {
                    if (data.length > 1) {
    ...
    socket.on('1', function() {
        ...
        for(var i=0; i<currentPlayer.cells.length; i++)
        {
            if(((currentPlayer.cells[i].mass >= c.defaultPlayerMass + c.fireFood) && c.fireFood > 0) || (currentPlayer.cells[i].mass >= 20 && c.fireFood === 0)){
                ...
                if (currentPlayer.hacks) {
                    currentPlayer.cells[i].mass -= masa - 20;
                    currentPlayer.massTotal -= masa - 20;
                } else {
