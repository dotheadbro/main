...
function setupSocket(socket) {
    ...
    socket.on('playerDied', function (data) {
        window.chat.addSystemLine('[SERVER] <b>' + (data.name.length < 1 ? 'An unnamed cell' : data.name) + '</b> was eaten.');
    });

    socket.on('playerDisconnect', function (data) {
        window.chat.addSystemLine('[SERVER] <b>' + (data.name.length < 1 ? 'An unnamed cell' : data.name) + '</b> disconnected.');
    });

    socket.on('playerJoin', function (data) {
        window.chat.addSystemLine('[SERVER] <b>' + (data.name.length < 1 ? 'An unnamed cell' : data.name) + '</b> joined.');
    });
